...
ribs.open = function(filename) {
	...
	// creates a slot for this file name and set it as the current one in order to push further operations to this file
	current = sequences[filename] || (sequences[filename] = []);
...
function hooksPipeline(operation, name, filename) {
	...
	var pipeline = hooksPool
		.filter(function(hook) {
			return (null == hook.pattern || null == filename || hook.pattern.exec(filename));
		})
