...
hooks.cropConstraintsHook = function(params, image) {
	...
	if (landmark)
		utils.computeAnchor(landmark, srcWidth, srcHeight, params);

	// default mode to anchor value to avoid useless repetition
	if (landmark && !anchor) anchor = landmark;

	// compute crop region origin
	utils.computeRegionOrigin(anchor, dstWidth, dstHeight, params.x, params.y, params);
