 ...
 * It is one of the following values (non order sensitive):
 *  - `tl`: top left
 *  - `t`: top center
 *  - `tr`: top right
 *  - `r`: center right
...
utils.computeAnchor = function(anchor, width, height, coords) {
	...
	if ('string' != typeof anchor || 0 === anchor.length || anchor.length > 2) {
		coords.x = Math.round(width / 2);
		coords.y = Math.round(height / 2);
		return;
	}
	...
	// x axis alignment
	if (~anchor.indexOf('l')) coords.x = 0;
	else if (~anchor.indexOf('r')) coords.x = width;
	else coords.x = Math.round(width / 2);

	// y axis alignment
	if (~anchor.indexOf('t')) coords.y = 0;
	else if (~anchor.indexOf('b')) coords.y = height;
	else coords.y = Math.round(height / 2);
};
