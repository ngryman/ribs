...
Pipeline.prototype.use = function(name, params) {
	...
	try {
		...
		this.queue.push(invokeOperation.bind(this, operation, params, Pipeline.hooks));
	}
	catch(err) {
...
Pipeline.prototype.done = function(callback) {
	...
	// EventEmitter throws an error when there is no listener for this particular event. We don't
	// want that!
...
Pipeline.hook = function(operation, name, hook) {
	...
	check('hook', hook, false, 'function');
 ...
 * @param callback
 */
function invokeOperation(operation, params, hooks, image, callback) {
	// `open` operation has only a callback as argument
	var openOp = 'function' == typeof image,
		finalParams;

	// `operation:before` event
	this.emit('operation:before', operation.name, params);

	// juggle arguments for `open` operation
	if (openOp) callback = image;
	...
	finalParams = operation.call(this,
		params,
		hooks,
		openOp ? wrappedCallback : image,
		openOp ? undefined : wrappedCallback);
}

/**
 * Register and proxy core operations.
 */

_.forOwn(operations, function(operation) {
	Pipeline.add(operation.name, operation);
});
